...
**Manmade cluster (5 left):** BANNER → TROPHY → ROBOTIC → CAMERA → INSTRUMENTS *(confirmed)*

**Phenomena cluster (7):** SHADOW-CAST → BUBBLE → SMOKE → PERFUME → HOLOGRAM → EXPLOSIVE → WEATHER/ATMOSPHERIC

**Roadmap totals:** 34 batches in roadmap (33 firm + INSTRUMENTS confirmed-pending-entries); 22 roadmap done (STATUE landed). **24 categories committed** (22 roadmap + 2 bonus PLANTS/FUNGI). **13 batches remaining** in roadmap: 4 Manmade + INSTRUMENTS confirmed-pending + 7 Phenomena. INSTRUMENTS reframed from "tentative" to "confirmed batch — entries deferred to future session" earlier this session.
...
| Manmade | IN PROGRESS 9/14 | ARCHITECTURE + VEHICLE + TOY + GAME-PIECE + JEWELRY + CONTAINER + TOOL + APPLIANCE + STATUE done. INSTRUMENTS 14th slot confirmed (entries pending future session). Next: BANNER. |
