...
- [ ] **Story (flesh out before final code-analysis): Rex's double life → "Rex Shadow."** Character-arc idea to develop: **Rex** (Gym Leader #1, Normal type) resents being written off as "just the first / easiest leader" and, craving the respect he's denied, secretly joins the **Umbra Order** — rising fast to become **"Rex Shadow."** *Data already supports this:* the Umbra commander with id **`umbra_commander_rex_shadow`** currently displays as **Commander Vorn** — reconcile (is "Vorn" an alias? a cover name the player later unmasks as Rex? or rename to lean into the reveal). Flesh out: subtle early-game foreshadowing (his bitterness in gym dialogue), the mid/late-game reveal, his Umbra team vs. his gym team, branching dialogue, and whether/how the player confronts both identities. **This is a story-analysis + narrative-design pass, to run before the final code-analysis stage.**
