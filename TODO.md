...
- [x] **`archetype-subtypes.md`** — transcription + brainstorm pass through all 15 existing sections complete (Mammals → Cosmological/Energy). Dragon renames + dog-breed additions + Mythical Batches A & B applied. PR #38 (transcription) merged; PR #39 (brainstorm pass) merged. **Phase 1 new-top-level-category roadmap locked at 34 batches** on branch `claude/brainstorm-archetypes-phase1-xiqpe` (last commit `5929357`). **16 categories committed so far** in order: PLANTS/FLORA, FUNGI/MUSHROOMS, PREHISTORIC/EXTINCT MEGAFAUNA, MICROORGANISMS, EGG/COCOON/CHRYSALIS, SHELL/CARAPACE/EXOSKELETON, SYMBIOSIS/PAIRED-BEINGS, CAVE/UNDERGROUND, URBAN, GLACIER, HOT-SPRING, ISLAND, CLAY/CERAMIC, THREAD/CLOTH, INK/PIGMENT, METAL-OBJECT. **Bio cluster done (5/5). Habitat cluster done (5/5, RIVER dropped). Materials cluster done (4/4). 20 batches remaining** in run order: Manmade (13: ARCHITECTURE, VEHICLE, TOY, GAME-PIECE, JEWELRY, CONTAINER, TOOL, APPLIANCE, STATUE, BANNER, TROPHY, ROBOTIC, CAMERA) → Phenomena (7: SHADOW-CAST, BUBBLE, SMOKE, PERFUME, HOLOGRAM, EXPLOSIVE, WEATHER/ATMOSPHERIC). Each batch follows propose → letter-picks → write → commit → push. *(RIVER dropped from Habitat cluster per 2026-05-11 session decision.)* Side note: quagga moved from PREHISTORIC into MAMMALS → Equids alongside zebra/zonkey; Myriapods sub-header (centipede, millipede) added inside INSECTS.
