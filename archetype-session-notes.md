...
**Phenomena cluster (1 left):** WEATHER/ATMOSPHERIC *(SHADOW-CAST + BUBBLE + SMOKE + PERFUME + HOLOGRAM + EXPLOSIVE landed 2026-05-13; mirage reserved as confirmed-pick per HOLOGRAM cross-section)*

**Roadmap totals:** 33 batches in roadmap (all 33 firm); 32 roadmap done (EXPLOSIVE landed 2026-05-13). **34 categories committed** (32 roadmap + 2 bonus PLANTS/FUNGI). **1 batch remaining** in roadmap: WEATHER/ATMOSPHERIC. BANNER permanently dropped 2026-05-12 session (parallels RIVER + URBAN drops in earlier Phase 1 work).
...
| Phenomena | IN PROGRESS 6/7 | SHADOW-CAST + BUBBLE + SMOKE + PERFUME + HOLOGRAM + EXPLOSIVE landed 2026-05-13. BUBBLE resolved TOY's balloon + balloon-animal deferrals. SMOKE routed volcanic-smoke/ash-cloud/dust-cloud to NATURAL-DISASTER-side, mist/fog to WEATHER-side, incense-smoke to PERFUME-side. PERFUME resolved SMOKE's incense-smoke routing. HOLOGRAM routed mirage-being to WEATHER-side as reserved confirmed-pick. EXPLOSIVE established weapons-scale-allowed convention (diverges from TOOL firearm-drop precedent). Remaining deferrals targeting Phenomena: snowdrift / icicle-formation / polar-night → WEATHER; mirage → WEATHER (confirmed-pick reservation). |
