...
- [ ] **Post-game-type move identity** *(deferred here from the move-effect rebalance pass, branch `claude/move-balance-NcbI2`)* — the 26-type move-effect rebalance gave every regular type's flat `effect:null` damaging moves a thematic minor effect / gimmick (multi-hit, stat-mod, status chance, crit, etc.). The 5 post-game types were intentionally held for this audit because their move-pool assignments aren't stable until the Typing review (bullet 1) settles which Forgotten carry the rare types. Remaining flat `effect:null` damaging moves needing identity: **Stellar 20 · Chrono 17 · Crystal 13 · Aether 9 · Primal 11 (70 total)**. When picking up:
...
- [ ] **Team Umbra + Lumoria main-cast appearance designs** — brainstorm + design: (1) **Team Umbra** (villain org) — its **admins** and its **leader**; (2) **Lumoria's main cast** — the **16 gym acolytes**, the **elite members**, and the **champion**. Same goal as the wielders — distinctive, good-looking character designs with clear silhouette + color identity.
...
- [🚧] **Inverse Mode (reversed type chart) — NG+ option.** A subsequent-playthrough option that **completely reverses the type chart**: super-effective ↔ not-very-effective, and immunities (0×) become weaknesses (2×). **Core implemented** (`inverseEffMult` in `js/data.js`; hooked into `getMoveEffectiveness` in `js/battle.js`; `G.inverseMode` persisted in save + migration; prompted at **Start New Game+** in `startNGPlus`). **Remaining:** in-battle UI indicator/badge when active; surface it in any type-matchup helper text; optional: a settings toggle outside NG+; verify AI/teambuilder still behaves sensibly under inversion.
- [ ] **Move relearner (relearning moves) implementation.** Let a Lumori re-learn any move from its learnset that it could already have learned (level ≤ current level) but doesn't currently know — standard "move reminder" NPC/menu. Define: where it lives (NPC in a town / party menu option), any cost (free, or an item/currency), eligible-move list = learnset entries with `level ≤ mon.level` not already in `moves`, and the swap UI (replace one of the 4 known moves). Tie into the existing party/summary screens.
