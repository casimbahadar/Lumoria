...
- [x] **`archetype-subtypes.md`** — transcription + brainstorm pass through all 15 existing sections complete (Mammals → Cosmological/Energy). Dragon renames + dog-breed additions + Mythical Batches A & B applied. PR #38 (transcription) merged; PR #39 (brainstorm pass) merged. **Phase 1 new-top-level-category roadmap locked at 33 batches (32 firm + INSTRUMENTS confirmed-pending-entries)** (35 − RIVER − URBAN − BANNER all permanently dropped, +INSTRUMENTS confirmed). **45 categories committed** (post-consolidation 2026-05-15). Originally 50; all 5 consolidations applied: MATHEMATICS + WRITING/SYMBOLS/LANGUAGE → **SYMBOLS & GLYPHS** (Combine A); CANDY/SWEETS + DRINKS/BEVERAGES → **FOOD & DRINK** (B); GLACIER + HOT-SPRING → **THERMAL EXTREMES** (C, Habitat 3 cats); CAMERA + HOLOGRAM → **OPTICAL IMAGING** (D, 5 sub-groups); TROPHY absorbed into JEWELRY as **Awards / honors** sub-group (5A, Manmade 12 cats). **PHASE 1 CATEGORY ROADMAP STILL COMPLETE (entry count unchanged at 427 for the 45 categories; 1,138 / 1,161 file-wide).**
