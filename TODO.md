...
- [x] **`archetype-subtypes.md`** — transcription + brainstorm pass through all 15 existing sections complete (Mammals → Cosmological/Energy). Dragon renames + dog-breed additions + Mythical Batches A & B applied. PR #38 (transcription) merged; PR #39 (brainstorm pass) merged. **Phase 1 new-top-level-category roadmap locked at 35 batches** on branch `claude/brainstorm-archetypes-phase1-xiqpe` (last commit `5929357`). **7 categories committed so far** in order: PLANTS/FLORA, FUNGI/MUSHROOMS, PREHISTORIC/EXTINCT MEGAFAUNA, MICROORGANISMS, EGG/COCOON/CHRYSALIS, SHELL/CARAPACE/EXOSKELETON, SYMBIOSIS/PAIRED-BEINGS. **Bio cluster done (5/5). 30 batches remaining** in run order: Habitat (6: CAVE, URBAN, RIVER, GLACIER, HOT-SPRING, ISLAND) → Materials (4: CLAY/CERAMIC, THREAD/CLOTH, INK/PIGMENT, METAL-OBJECT) → Manmade (13: ARCHITECTURE, VEHICLE, TOY, GAME-PIECE, JEWELRY, CONTAINER, TOOL, APPLIANCE, STATUE, BANNER, TROPHY, ROBOTIC, CAMERA) → Phenomena (7: SHADOW-CAST, BUBBLE, SMOKE, PERFUME, HOLOGRAM, EXPLOSIVE, WEATHER/ATMOSPHERIC). Each batch follows propose → letter-picks → write → commit → push. Side note: quagga moved from PREHISTORIC into MAMMALS → Equids alongside zebra/zonkey; Myriapods sub-header (centipede, millipede) added inside INSECTS.
