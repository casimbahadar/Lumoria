...
- [ ] **End-of-audit (next): move-key rename pass.** Other session renamed `MOVES_DATA[].name` display fields but left snake_case keys unchanged (e.g. `ancient_power:` key → display "Primeval Force"). To clean up the gap before final code review, do a focused sweep renaming keys to match new display names. Touches MOVES_DATA + every learnset (~5,500 refs) + trainer `moves:[...]` arrays + battle/AI hardcodes. Must be atomic — write a key-rename script that updates all refs simultaneously.
