...
- [ ] **Step 2:** Create new type-effectiveness chart with surprise twists — needs approval before applying.
  ...
  - **Phase E (balance audit after Phases A-D complete):** with all 26 type rows + columns locked, run a type-strength audit — compare each type's offensive profile (count of 2× / 0.5× / 0× attacks) and defensive profile (count of weaknesses / resistances / immunities) to identify outliers (types that are over-dominant or under-powered). Propose adjustments to specific cells for user approval. Goal: no single type vastly outshines or underperforms others. Runs after the chart is fully filled, before moving to Step 3.
