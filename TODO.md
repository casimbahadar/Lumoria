...
- [x] **`archetype-subtypes.md`** — transcription + brainstorm pass through all 15 existing sections complete (Mammals → Cosmological/Energy). Dragon renames + dog-breed additions + Mythical Batches A & B applied. PR #38 (transcription) merged; PR #39 (brainstorm pass) merged. **Phase 1 new-top-level-category roadmap locked at 33 batches (32 firm + INSTRUMENTS confirmed-pending-entries)** (35 − RIVER − URBAN − BANNER all permanently dropped, +INSTRUMENTS confirmed). **45 categories committed** (post-consolidation + dedup 2026-05-15). 5 consolidations applied (Combines A-D + 5A); 12 cross-cluster dedup cases applied (A1-A12 — see `archetype-session-notes.md` for case-by-case explanations); A11 Option A follow-up applied (new `prophet-being / oracle-prophet` entry added to MYTHICAL CREATURES Other to capture divine-flavor). File-wide entry total: 1,138 → 1,128 (1,151 with dragons). **PHASE 1 CATEGORY ROADMAP STILL COMPLETE.**
