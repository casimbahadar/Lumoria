...
| 📉 Standalone count reduction | ✅ done (2026-06-12) | Reducible NG+ standalones 70→56 via golem / monolith / dragon-pair merges (id-ascending lines, caps respected). See "🧩 Standalone-count reduction (#6)" checkpoint below. |
| 🏷️ Luminex renaming + final lore | ⏳ folded into capstone | PRs #34–35; Tier-1 confusables done (Cerebraith/Narwhirl/Cobravyrm). **Tier-2 + affix-collision renaming (shared prefix/suffix near-twins — Quarrix/Quarrex, Mirkling/Mistling, etc.) is now folded into the 📋 Final per-Lumori capstone's per-entry `name` review** — prefixes/suffixes get diversified there as part of the last-step pass, not as a separate piecemeal pass. Catalog + guidance in the UNIFIED checkpoint (affix-collision catalog, lines ~56–62). |
...
| 📋 Final per-Lumori complete analysis | ⏳ not started | **RUN LAST.** Rescoped 2026-06-10 from the old stats-only "Final stat-spread review" → a comprehensive per-Lumori final pass: re-check **everything** one more time for each entry — stats/BST spread, typing (+ cap), lore, desc, emoji, archetype, moveset/learnset, evolution-line logic, name — and produce a **detailed written explanation per family and per standalone** (what it is, why each field/decision is right, any residual flags). The per-entry **`name`** review here is where the Tier-2 / shared prefix-suffix **affix collisions** (Quarrix/Quarrex, Mirkling/Mistling, etc.) get resolved and diversified — i.e. the 🏷️ renaming pass lives inside this step. Family-by-family / standalone-by-standalone format. This is the capstone verification of all prior audit work. |
...
- `-vast` group (suffix retired entirely, all 12 renamed incl. the Forgotten one): #52 Permavast→**Aurochill** · #65 Mycovast→**Plaguecap** · #71 Verdovast→**Groveguard** · #94 Petrovast→**Basaltback** · #97 Tectonvast→**Quakehide** · #167 Psychovast→**Psymastiff** · #184 Behemovast→**Ursamight** · #276 Scorchvast→**Magmaw** · #282 Mechavast→**Mechabrute** · #363 Chromavast→**Chromaton** · #394 Solarvast→**Heliogriff** · #489 Forgotten Ironvast→**Forgotten Carapax** (2026-06-14). #184 analyzed for lore/type coherence first — Normal/Fighting brawler-bear confirmed consistent, no rewrite needed.
...
- `-thon`/`-mite`/`-puff`/`-rown`/`-don` (each → 3 pre-462, one rename apiece): #193 Rugothon→**Cliffpinch** · #277 Magmite→**Magmote** · #293 Gustpuff→**Windwhelp** · #358 Pyrocrown→**Psyphoenix** (relieves Pyr-) · #26 Aquidon→**Brinecrush** (2026-06-14). [`-aith` left as-is: 3 pre-462 + Forgotten Dreamaith.]
